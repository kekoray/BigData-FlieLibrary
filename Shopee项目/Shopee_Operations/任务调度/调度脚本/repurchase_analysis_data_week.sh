...
#########################################################
# Description :复购统计周维度统计,自然周，每周跑一次
# Script   :repurchase_analysis_data_week.sh
# Version  :1.0
# Date     :2020-11-10
# Author   :koray
# Company  :menglar
...
sql="delete from repurchase_analysis_data_week where date='${timest}'"
...
,t2.role_id
,t2.product_id
,nvl(count(distinct case when t1.dt>='${pre_week_start}' and t1.dt<='${pre_week_end}' then t1.uid end),0) as repurchase_num
,nvl(count(distinct case when t1.dt>='${bef_pre_week_start}' and t1.dt<='${bef_pre_week_end}' then t1.uid end),0) as past_repurchase_num
,nvl(sum(case when t1.dt>='${pre_week_start}' and t1.dt<='${pre_week_end}' then t1.amount end),0) as gmv 
,nvl(sum(case when t1.dt>='${bef_pre_week_start}' and t1.dt<='${bef_pre_week_end}' then t1.amount end),0) as past_gmv 
,nvl(count(distinct case when t1.dt>='${pre_week_start}' and t1.dt<='${pre_week_end}' and t1.turn_type='1' then t1.uid end),0) as artificial_change
,nvl(count(distinct case when t1.dt>='${pre_week_start}' and t1.dt<='${pre_week_end}' and t1.turn_type='0' then t1.uid end),0) as artificial_change
,'${timest}'
from
(
select
...
#ExportToMySQLByHQL "$hql" "${DT[0]}" "repurchase_analysis_data_week" "common_bus"
...
SqoopDirectExportToMySQLByHQL "shopee_operations" "$hql" "${DT[0]}" "repurchase_analysis_data_week" 
